        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            {activeSection === 'orders' && (
              <div className="bg-[#242424] rounded-2xl p-8 border border-gray-800">
                <h2 className="text-2xl font-bold text-white mb-6">My Orders History</h2>
                {myOrders.length === 0 ? (
                        ...
                        <div className="space-y-1 mb-3">
                          {order.items.map((item, idx) => (
                            <p key={idx} className="text-gray-400 text-sm">
                              {item.quantity}x {item.name} — {(item.price * item.quantity).toFixed(2)} MDL
                            </p>
                          ))}
                        </div>
                    ...
                    {myReservations.map((res) => (
                      <div key={res.id} className="bg-[#1a1a1a] rounded-xl p-5 border border-gray-700">
                        <div className="flex justify-between items-start">
                          <div>
                            <p className="text-white font-bold">Table {res.table}</p>
